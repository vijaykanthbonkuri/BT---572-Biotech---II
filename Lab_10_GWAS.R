...
`GAPIT.Manhattan` <-
  function(GI.MP = NULL,GD=NULL,name.of.trait = "Trait",plot.type = "Genomewise",width0=13,height0=5.75,
    ...
    sp=sort(GI.MP[,3])
    spd=cutOff-sp*numMarker/1:length(sp)
    # print(min(spd))
    if(length(spd[spd<0])==length(spd)) 
    {
      FDRcutoff=NA
    }else{
      index_fdr=grep(min(spd),spd)[1]+1
  ...
  function(PWI = PWI, FDR.Rate = 0.05, FDR.Procedure = "BH"){
    ...
    {  
      ...
      if(dim(PWI)[1] > 1){ 
        ...
        res <- mt.rawp2adjp(PWI[,4], alpha=FDR.Rate,FDR.Procedure)
...
#=============================================================================================
`GAPIT.Power.Compare` <-function(GD=NULL,GM=NULL,KI=NULL,Y=NULL,CV=NULL,PCA.total=0,rep=NULL,h2=NULL,NQTN=NULL,WS=1,maxOut=100,Method=NULL){
  # Object: compare to Power against FDR for GLM,MLM,CMLM,ECMLM,SUPER
  # rep:repetition times
  # Authors: You Tang & Jiabo Wang
  # Last update: Feb 1, 2020 
  ############################################################################################## 
  if(is.null(GD)){stop("Read data Invalid. Please select read valid flies !")}
  ...
  if(is.null(Method)) stop("Please choose methods to compare !!")
  MM=length(Method)
  X<-GD[,-1]
  ...
  for(i in 1:rep)
  {
    ...
    mysimulation<-GAPIT.Phenotype.Simulation(GD=GD,GM=GM,h2=h2,NQTN=NQTN)
    posi=mysimulation$QTN.position
    myY=mysimulation$Y
    print("Done to simulate Phenotype with genotype !!")
    for(j in 1:MM)
    {
      print(paste("*****************","GWAS by GAPIT...",Method[j]," model totle:",rep,sep=""))
      #--------------------------
      myGAPIT=GAPIT(
        Y=myY,
        # G=G,
        ...
        model=Method[j],
        file.output=F)
      power<-GAPIT.Power(WS=WS, alpha=c(.01,.05,.1,.2,.3,.4,.5,.6,.7,.8,.9,1), maxOut=maxOut,seqQTN=posi,GM=GM,GWAS=myGAPIT$GWAS)
      rep.power.GLM[,j]<-rep.power.GLM[,j]+power$Power
      rep.FDR.GLM[,j]<-rep.FDR.GLM[,j]+power$FDR
      rep.Power.Alpha.GLM[,j]<-rep.Power.Alpha.GLM[,j]+power$Power.Alpha
    }
  ...
  write.csv(rep.power.GLM,paste(h2,"_",NQTN,".Power.GLM",rep,".csv",sep=""))
  write.csv(rep.FDR.GLM,paste(h2,"_",NQTN,".FDR.GLM",rep,".csv",sep=""))
  write.csv(rep.Power.Alpha.GLM,paste(h2,"_",NQTN,".Alpha.GLM",rep,".csv",sep=""))
  ...
  pdf(paste("GAPIT.Power.Compare with ",mMethod," in Simulated", ".pdf", sep = ""), width = 4.5, height = 4.5,pointsize=9)
  par(mar = c(5,6,5,3))
  #win.graph(width=6, height=4, pointsize=9)
  #palette(c("blue","red","green4","brown4","orange",rainbow(5)))
  palette(rainbow(MM))
  plot(rep.FDR.GLM[,1],rep.power.GLM[,1],bg="lightgray",xlab="FDR",ylab="Power",ylim=c(0,1),xlim=c(0,1),main="Power against FDR",type="o",pch=20,col=1,cex=1.0,cex.lab=1.3, cex.axis=1, lwd=2,las=1)
  if(MM!=1)
  {
    for(i in 2:MM)
    {
      lines(rep.power.GLM[,i]~rep.FDR.GLM[,i], lwd=2,type="o",pch=20,col=i)
    }
  }
  # lines(rep.power.CMLM[,6]~rep.FDR.CMLM[,6], lwd=2,type="o",pch=20,col=3)
  # lines(rep.power.MLM[,6]~rep.FDR.MLM[,6], lwd=2,type="o",pch=20,col=4)
  # lines(rep.power.GLM[,6]~rep.FDR.GLM[,6], lwd=2,type="o",pch=20,col=5)
  legend("bottomright",c(Method), pch = 20, lty =1,col=c(1:MM),lwd=2,cex=1.0,bty="n")
  #
  ...
  pdf(paste("GAPIT.Type I error_Power ",mMethod," in Simulated", ".pdf", sep = ""), width = 6, height = 4.5,pointsize=9)
  par(mar = c(5,6,5,3))
  #win.graph(width=6, height=4, pointsize=9)
  #palette(c("blue","red","green4","brown4","orange",rainbow(5)))
  palette(rainbow(MM))
  plot(myalpha1,rep.Power.Alpha.GLM[,1],log="x",bg="lightgray",xlab="Type I error",ylab="Power",main="Power against FDR",type="o",pch=20,col=1,cex=1.0,cex.lab=1.3, cex.axis=1, lwd=2,las=1,ylim=c(min(rep.Power.Alpha.GLM),max(rep.Power.Alpha.GLM)))
  ...
  if(MM!=1)
  {
    for(i in 2:MM)
    {
      lines(rep.Power.Alpha.GLM[,i]~myalpha1, lwd=2,type="o",pch=20,col=i)
    }
  }
  legend("bottomright",c(Method), pch = 20, lty =1,col=c(1:MM),lwd=2,cex=1.0,bty="n")
  #
...
emma.eigen.R.wo.Z <- function(K, X) {
  ...
  {
    S <- diag(n)-X%*%ginv(crossprod(X,X))%*%t(X)
  }
...
#myGAPIT=GAPIT(
 # Y=myY[,c(1,2)],      # Phenotypes: fist column is genotype ID and second is a phenotype
  ...
  model=c("GLM"))  	# Select a statistical model
#View(myGAPIT$GWAS)
#getwd()
...
# Load result
#res <- read.csv("GAPIT.GLM.EarHT.GWAS.Results.csv",  header = T)
#head(res)

# Manhattan plot
#manhattan(res, main="Ear height", chr = "Chromosome", bp = "Position", p = "P.value", snp = "SNP",           
          #col = c("blue", "orange"),           
          #genomewideline = 4.5,  highlight = "PZA03188.4"  )
# Save the plot
...
#getwd()
# Create a new folder
dir.create("GWAS_NewTrait", showWarnings = FALSE)

## Activity##
# Set as working directory
setwd("GWAS_NewTrait")
myGAPIT2 <- GAPIT(
  Y = myY[, c(1, 3)],   # 1 = genotype ID, 3 = new phenotype
  GD = myGD,
  GM = myGM,
  PCA.total = 3,
  model = "GLM"
...
res <- read.csv("GAPIT.Association.GAPIT.GLM.EarHT.GWAS.Results.csv", header = TRUE)
...
res <- read_excel("GAPIT.GLM.EarHT.GWAS.Results.")
library(readxl)

res <- read_excel("GAPIT.GLM.EarHT.GWAS.Results.xlsx")
getwd()
list.files()
...
res <- read_excel("GAPIT.GLM.EarHT.GWAS.Results.xlsx")
res <- read.csv("GAPIT.GLM.EarHT.GWAS.Results.csv", header = TRUE)
head(res)
colnames(res)
top_snp <- res$SNP[which.min(res$P.value)]
top_snp
library(qqman)

manhattan(
  res,
  main = "Ear Height GWAS",
  chr = "Chromosome",
  bp = "Position",
  p = "P.value",
  snp = "SNP",
...
library(qqman)
png("Manhattan_EarHT.png", width = 10, height = 5, units = "in", res = 300)
